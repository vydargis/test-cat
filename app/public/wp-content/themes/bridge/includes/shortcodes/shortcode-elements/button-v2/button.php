...
class ButtonV2 implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                array(
					array(
						'type'        => 'dropdown',
						'param_name'  => 'type',
						'heading'     => esc_html__( 'Type', 'qodef-core' ),
						'value'       => array(
							esc_html__( 'Solid/Default', 'qode' )	=> '',
							esc_html__( 'Simple', 'qode' )			=> 'simple'
						),
						'admin_label' => true
					),
                    array(
               ...
               qode_icon_collections()->getVCParamsArray(array(), '', true),
                array(
					...
					array(
                        ...
                        'group'       => esc_html__('Design Options','qode'),
                        'description' => esc_html__('Only for solid type','qode')
					),
					array(
						'type'        => 'colorpicker',
                    ...
                    array(
                        'type'        => 'dropdown',
                        'heading'     => esc_html__('Font Style','qode'),
                        'param_name'  => 'font_style',
                        'value'       => array_flip(qode_get_font_style_array(true)),
                        'admin_label' => true,
                        'group'       => esc_html__('Design Options','qode')
                    ),
                    array(
    ...
    private function getButtonStyles($params) {
        ...
        if(!empty($params['font_family'])) {
            $styles[] = 'font-family: '. qode_get_font_option_val($params['font_family']);
        }
        ...
        if(!empty($params['letter_spacing'])) {
            $styles[] = 'letter-spacing: '.qode_filter_px($params['letter_spacing']).'px';
        }
